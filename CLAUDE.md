...
```bash
openclaw agent --agent main -m \
  "[회사명] 최신 동향 조사. 최근 6개월 주요 발표/전략 방향/채용 포커스/투자 현황. JD: [JD 텍스트]" --json
```
...
### ━━ PHASE 2. 전략 수립 ━━
> 에이전트: **Claude Sonnet (나)** — 판단/매칭
> 토큰 최적화: 서비스분석 요약본만 읽음 (전체 X)
...
```
→ `전략/[회사명]_전략.md` 저장
→ **사용자에게 전략 보고 후 승인 받고 PHASE 3 진행**

---

### ━━ PHASE 3. 콘텐츠 + 빌드 (병렬 실행) ━━
...
| Phase | 에이전트 | 모델 | 비용 |
|---|---|---|---|
| 1-A 서비스분석 | OpenClaw | Gemini 2.5 Flash | 무료 |
| 1-B JD분석 | OpenClaw | Gemini 2.5 Flash | 무료 |
| 2 전략수립 | Claude Sonnet (나) | Sonnet 4.6 | 유료 |
| 3-A 카피라이팅 | copywriting 스킬 | — | — |
| 3-B 레이아웃 | presentation-design 스킬 | — | — |
| 3-C HTML 빌드 | frontend-slides + web-coder | Sonnet 4.6 | 유료 |
| 4-B 비판자 | Codex → Groq fallback | o3 → Llama | 무료 |
